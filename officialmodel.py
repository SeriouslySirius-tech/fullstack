...
class BardGenerator:
    ...
    def format_text(self, text):
        # print(text)
        question = text[text.find("Question:")+len("Question:"):text.find('?')]
        options = text[text.find("A)"):text.find("Answer:")]
    ...
    def generate_questions_from_text_single_word(self, topic, difficulty="college"):
        single_word_response = self.text_model.generate_content(f"Give me 1 question that can be answered in a single word of {difficulty} difficulty with 4 choices on the topic {topic} and the answer to the question in the following example format: \
